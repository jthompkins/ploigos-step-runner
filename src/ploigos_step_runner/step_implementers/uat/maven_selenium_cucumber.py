...
class MavenSeleniumCucumber(MavenGeneric):
    ...
    def __parse_surefire_report_for_evidence(self, surefire_report_path, step_result):
        """
        ...
        #xml_files = os.listdir(surefire_report_path)
        xml_files = glob.glob(surefire_report_path + '/*' + file_ext, recursive=False)
        for file in xml_files:
            try:
                pom_testsuite_element = get_xml_element(file, xml_element)
            except ValueError as error:
                raise StepRunnerException(f"Error parsing surefire report for evidence: {error}") from error

            time = pom_testsuite_element.attrib["time"]
            tests = pom_testsuite_element.attrib["tests"]
            errors = pom_testsuite_element.attrib["errors"]
            skipped = pom_testsuite_element.attrib["skipped"]
            failures = pom_testsuite_element.attrib["failures"]
